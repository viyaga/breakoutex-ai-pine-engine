...
export async function fetchActivePineBots(): Promise<PineBotConfig[]> {
    ...
    const configs: PineBotConfig[] = raw.map(bot => {
        ...
        return {
            ...
            PRODUCT_TYPE:              bot.PRODUCT_TYPE || (bot.MARKET_TYPE === 'indian_equity' ? 'MIS' : 'NRML'),
            ...
            IS_INTRADAY_SQUARE_OFF_ENABLED: bot.IS_INTRADAY_SQUARE_OFF_ENABLED !== false && ['indian_stock', 'indian_futures', 'indian_equity'].includes(bot.MARKET_TYPE || ''),
            INTRADAY_SQUARE_OFF_TIME:  bot.INTRADAY_SQUARE_OFF_TIME || '15:15',
