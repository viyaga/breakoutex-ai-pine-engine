...
export async function fetchActivePineBots(): Promise<PineBotConfig[]> {
    ...
    const configs: PineBotConfig[] = raw.map(bot => {
        ...
        return {
            ...
            MARKET_TYPE:               bot.MARKET_TYPE || (['zerodha', 'angelone'].includes(bot.EXCHANGE?.toLowerCase()) ? 'indian_stock' : ['alpaca'].includes(bot.EXCHANGE?.toLowerCase()) ? 'us_equity' : 'crypto'),
            ...
            PRODUCT_TYPE:              bot.PRODUCT_TYPE || (['indian_equity', 'us_equity'].includes(bot.MARKET_TYPE || '') ? 'MIS' : 'NRML'),
            IS_TRAILING_SL_ENABLED:    Boolean(bot.IS_TRAILING_SL_ENABLED),
            TRAILING_SL_CALLBACK_PERCENT: Number(bot.TRAILING_SL_CALLBACK_PERCENT ?? 1.0),
            IS_INTRADAY_SQUARE_OFF_ENABLED: bot.IS_INTRADAY_SQUARE_OFF_ENABLED !== false && ['indian_stock', 'indian_futures', 'indian_equity', 'us_equity', 'us_options'].includes(bot.MARKET_TYPE || ''),
            INTRADAY_SQUARE_OFF_TIME:  bot.INTRADAY_SQUARE_OFF_TIME || ((bot.MARKET_TYPE || '').startsWith('us_') ? '15:45' : '15:15'),
